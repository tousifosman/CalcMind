...
// persisted, never undoable). Currently holds one setting: the numeral font size (§1.2
// `tokens.numeralFontSize`'s live override, added for the Settings sheet).
//
...
export const usePreferencesStore = create<PreferencesState>((set, get) => ({
  ...
  setNumeralFontSize: (size) => {
    ...
    preferencesAdapter.write({ numeralFontSize: clamped }).catch(() => {
