...
> **Status:** partially implemented. P0 (foundations) and P1 (canvas pan/zoom) are built —
...
criteria and sequencing notes now live there, expanded into tasks with their own objectives,
architecture references and acceptance criteria, and marked with what is already done.

This section number is kept so existing citations of §16 and §17 stay correct.

The split is deliberate: this document describes the design and is rewritten in place as the
design changes, whereas the plan is a progress record that gets ticked off. Mixing the two made
it unclear which parts of the file were claims about the present and which were intentions.
