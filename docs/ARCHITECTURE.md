...
- Connectors are drawn for **all** links by default, not only the selected one. Tydlig hides them
  ...
  every live reference gets a cubic in the source's identity hue with an SVG arrowhead. Curves sit
  at `CONNECTOR_UNSELECTED_OPACITY` by default and only reach full strength while they, or their
  source/reference cell, are the current selection — losing selection fades everything back down
  rather than leaving prior curves vivid (decision #35). Dangling refs draw no curve (§11.2 owns
  their chrome).
...
| 35 | Connector curves default to `CONNECTOR_UNSELECTED_OPACITY` (vague) and only rise to full opacity while selection makes them relevant, superseding decision #13's "full opacity unless something else is selected" | User-reported: with nothing selected every link read at full strength, and selecting one cell only dimmed the *others* — so the emphasis a selection buys was only visible as a relative fade, and it reset to uniformly vivid the moment focus left the cell, rather than the graph reading calm by default with selection as the thing that lights a path up | A user wants recently-touched links to stay emphasized after selection moves on, not just the live selection |
