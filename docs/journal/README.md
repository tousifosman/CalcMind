...
| [2026-08-12](2026-08-12.md) | `Create link` keypad button; decimal/`+/-` relocated into the digit grid's `0` row; `()` moved under `+`; `Add components`/`Notes` placeholders; `Keypad.test.tsx` `allSelected` reset bug fixed; `/prpreview` PR-comment trigger for `deploy-pr-preview.yml` |
