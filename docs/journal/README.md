...
| [2026-08-19](2026-08-19.md) | Follow-up on 2026-08-17's pin feature, three rounds in one day: (1) fixed a resize-on-pin bug (the drag handle was conditionally *mounted* rather than always-mounted-and-repainted, decision #22), recoloured the connector line to the popover's own chrome, added a Step field (default `0.1`, `quantizeToStep`) that dragging snaps to with a vibration per step; (2) tightened top padding and swapped a plain colour-change "tick" for an actual `CheckIcon` glyph; (3) two real-device-only position bugs — the popover shifted as the scrubbed value's digit count changed (fixed by freezing its anchor width per node-open) and the connector visibly detached from the cell while panning until the gesture released (fixed by a new `uiStore.liveViewport` bridge so screen-space UI outside `Canvas`'s own transform can track a gesture live instead of only the release-throttled committed viewport, decision #23). Each round's own live-verification pass caught the *next* round's bug class by widening what it checked — bounding boxes, then real screenshots at zoom, then mid-gesture (not just before/after) screenshots — a sequence worth reading in order |
| [2026-08-21](2026-08-21.md) | Connector curves now default to `CONNECTOR_UNSELECTED_OPACITY` (vague) and only reach full opacity while their link is the live selection — decision #24 supersedes the "full opacity unless something else is selected" half of decision #13, since the old default left connectors vivid at rest and only reset to vivid again on deselect, the opposite of the requested resting state |
