...
| [2026-08-19](2026-08-19.md) | Follow-up on 2026-08-17's pin feature: fixed a resize-on-pin bug (the drag handle was conditionally *mounted* rather than always-mounted-and-repainted — a general pattern worth naming, decision #22), recoloured the connector line to the popover's own chrome instead of the cell's identity hue, and added a Step field (default `0.1`, `quantizeToStep` in `inferSliderRange.ts`) that continuous dragging now snaps to with one short vibration per step crossing. Caught, in the revisions section, that the prior session's own live-verification pass checked testid presence but never a bounding-box comparison, which is what would have caught the resize bug before shipping |
