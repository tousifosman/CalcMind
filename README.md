...
`.github/workflows/deploy-pr-preview.yml` publishes a given PR's build to a temp path — `https://<user>.github.io/<repo>/pr-preview/pr-<n>/` — without merging it, so it can be reviewed live before landing. Trigger it by commenting `/prpreview` on the PR (only commenters with write access to the repo can trigger it — anyone else's comment is silently ignored); it reacts 👀 on the comment to confirm it picked up the command, then comments the preview URL back once deployed. It can also be run manually from the Actions tab (`Deploy PR preview` → `Run workflow`), giving it the PR number. The preview is torn down automatically when the PR closes, or on demand with a `/prpreview remove` comment (or by re-running the workflow with the `remove` action).
